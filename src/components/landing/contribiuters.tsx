                ...
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {Array.from({ length: Math.min(6, maxContributors) }).map((_, i) => (
                            <div key={i} className="animate-pulse flex items-center gap-4 bg-white/60 p-4 rounded-xl">
                    ...
                    <div className="p-6 text-gray-500">No contributors found for the provided repositories.</div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {contributors.map((c, idx) => (
                            <a
                                ...
                                className="group block bg-white/80 hover:scale-[1.02] transition-transform duration-200 rounded-2xl p-4 shadow-sm"
                            >
                                <div className="flex items-center gap-4">
                                    <div className="relative">
                                        <img src={c.avatar_url} alt={c.login} className="w-14 h-14 rounded-full ring-2 ring-white shadow-sm" />
                                        <div className="absolute -right-2 -top-2 bg-gradient-to-br from-yellow-400 to-red-400 text-white text-xs font-bold px-2 py-0.5 rounded-2xl shadow">#{idx + 1}</div>
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between">
                                            <div className="truncate">
                                                ...
                                                <div className="text-xs text-gray-400">@{c.login}</div>
                                            </div>
                                            <div className="ml-3 text-right">
                                                <div className="text-sm font-medium">{c.contributions}</div>
                                        ...
                                        <div className="mt-3 flex items-center gap-3">
                                            ...
                                            <div className="w-8 text-xs text-gray-500 text-right">{Math.round((c.contributions / (maxContrib || 1)) * 100)}%</div>
                                        </div>
                                    </div>
                                </div>
