...
import {
  ...
  SiFigma, SiPostman, SiWebpack, SiVite, SiNpm, SiYarn
} from "react-icons/si"
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-between mb-4 pt-6 md:pt-8">
              ...
              {portfolio.twitterUrl && (
                  <motion.button
                  onClick={() => window.open(portfolio.twitterUrl, '_blank')}
                    ...
                    whileTap={{ scale: 0.98 }}
                  >
                    <Twitter className="h-5 w-5 md:h-6 md:w-6 group-hover:drop-shadow-lg" />
                  </motion.button>
              )}
              {portfolio.instagramUrl && (
                  <motion.button
                  onClick={() => window.open(portfolio.instagramUrl, '_blank')}
                    className="group p-3 rounded-xl bg-transparent border border-orange-500/30 text-orange-400 hover:bg-black/20 hover:border-orange-500/50 hover:text-orange-300 transition-all duration-300"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <Instagram className="h-5 w-5 md:h-6 md:w-6 group-hover:drop-shadow-lg" />
                  </motion.button>
                )}
                <motion.button
                  onClick={() => window.open(portfolio.githubUrl, '_blank')}
                  className="group p-3 rounded-xl bg-gray-800 border border-gray-700 text-gray-300 hover:bg-gray-700 hover:border-gray-600 hover:text-white transition-all duration-300"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <Github className="h-5 w-5 md:h-6 md:w-6 group-hover:drop-shadow-lg" />
                </motion.button>
              </motion.div>
            </div>
            ...
            <div>
              ...
              <motion.p 
                className="text-sm md:text-base text-gray-300 mb-2 font-medium"
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.8, delay: 0.6 }}
              >
      ...
      {portfolio.skills && portfolio.skills.length > 0 && (
        ...
        >
          <div className="mx-auto px-3 sm:px-4 max-w-screen-md">
            <motion.h2 
              className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-white"
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 md:gap-3">
              {portfolio.skills.map((skill, index) => {
                ...
                return (
                  ...
                  >
                    <div className="relative flex items-center p-2 md:p-2.5 bg-transparent border border-orange-500/30 rounded-lg hover:bg-black/20 hover:border-orange-500/50 transition-all duration-200">
                      <div className="flex-shrink-0 w-7 h-7 md:w-8 md:h-8 mr-2 md:mr-3 flex items-center justify-center">
                        <IconComponent 
      ...
      >
          ...
          <div className="flex justify-center gap-5 md:gap-6">
            {portfolio.twitterUrl && (
              <motion.button
                onClick={() => window.open(portfolio.twitterUrl, '_blank')}
                className="p-3 md:p-4 rounded-2xl backdrop-blur-xl bg-black/40 border border-orange-500/30 text-orange-400 hover:bg-orange-500/10 hover:text-orange-300 hover:border-orange-400/50 hover:shadow-lg hover:shadow-orange-500/20 transition-all duration-300"
            ...
            {portfolio.instagramUrl && (
              <motion.button
                onClick={() => window.open(portfolio.instagramUrl, '_blank')}
                className="p-3 md:p-4 rounded-2xl backdrop-blur-xl bg-black/40 border border-amber-500/30 text-amber-400 hover:bg-amber-500/10 hover:text-amber-300 hover:border-amber-400/50 hover:shadow-lg hover:shadow-amber-500/20 transition-all duration-300"
                whileHover={{ scale: 1.1, y: -2, rotate: -5 }}
                whileTap={{ scale: 0.9 }}
              >
            ...
            <motion.button
              onClick={() => window.open(portfolio.githubUrl, '_blank')}
              className="p-3 md:p-4 rounded-2xl backdrop-blur-xl bg-black/40 border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/10 hover:text-yellow-300 hover:border-yellow-400/50 hover:shadow-lg hover:shadow-yellow-500/20 transition-all duration-300"
              whileHover={{ scale: 1.1, y: -2, rotate: 5 }}
              whileTap={{ scale: 0.9 }}
            >
