...
import { saveAuthenticatedUser } from "@/lib/services"
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  try {
    ...
    try {
      ...
      const { savedUser, email } = await saveAuthenticatedUser(tokenData.access_token, userData)
      isNewUser = Boolean(!savedUser) // heuristic; keep original behavior below
      ...
      if (isNewUser && !userEmail.includes('@placeholder.com')) {
        ...
        const requestUrl = new URL(req.url)
        const baseUrl = `${requestUrl.protocol}//${requestUrl.host}`
        const welcomeHtml = generateWelcomeEmail({ name: userData.name || userData.login, username: userData.login, portfolioUrl: baseUrl })
        sendEmail({ to: userEmail, subject: "🚀 Welcome to DevFolio - Let's Build Your Portfolio!", html: welcomeHtml }).catch(() => {})
      } else if (!isNewUser) {
