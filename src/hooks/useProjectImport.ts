...
export function useProjectImport({
  ...
  }, [open, projectUrl, isFromGitHubSelection, captureScreenshot])

  const handleAddByGithub = useCallback((repo: RepositoryLike) => {
    setTitle(repo.name || "")
    ...
    async (event: SyntheticEvent<HTMLImageElement>) => {
      ...
      if (logo && logo.includes("/api/portfolio/github-og-image")) {
        try {
          ...
          // Keep current directLogoUrl when header fetch fails.
        }
